...
export class SupabaseRepo implements Repo {
  ...
  async asegurarMunicipioPorIne(ineCode: string): Promise<Municipio> {
    // ¿Ya está activado en la lista operativa?
    const { data: existente } = await this.svc
      .from("municipalities")
      .select("*")
      .eq("ine_code", ineCode)
      .maybeSingle();
    if (existente) return rowToMunicipio(existente);
    ...
    const { data: creado } = await this.svc
      .from("municipalities")
      .insert({
        slug: slugify(oficial.nombre),
        ...
        matricula_escolar: 0,
        umbral_escolar: 0,
        riesgo_despoblacion: "medio",
      })
      .select()
      .single();
    return rowToMunicipio(creado);
  }
